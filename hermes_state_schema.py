...
class SessionSchemaMixin:
    ...
    def _init_schema(self):
        ...
        # Wheelbase titles are deliberately non-unique. Reconcile the old
        # unique index on every writable open, including databases whose
    ...
    def _run_data_migrations(self, cursor: sqlite3.Cursor, current_version: int, fts5_available: bool) -> None:
        ...
        if current_version < 26:
            # Wheelbase v26 drops the title uniqueness constraint. Keep the
            # migration index-only and repeat the reconciliation in _init_schema
            # for stores reopened by an older binary.
            with contextlib.suppress(sqlite3.Error):
                cursor.execute("DROP INDEX IF EXISTS idx_sessions_title_unique")
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_sessions_title "
