...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def set_auto_title_if_empty(self, session_id: str, title: str) -> bool:
        """Back-compat shim: write an LLM title only if no equal/higher-authority
        title exists.
        ...
        source. The predicate-and-write still run in one transaction so a
        concurrent manual rename cannot be overwritten, but this does not
        enforce uniqueness — titles are not unique (schema v26).
        """
    ...
    def list_sessions_rich(
        ...
        if user_id:
            # Multi-user cloud gateway: scope the list to one owner. Legacy
            # rows with NULL user_id are intentionally excluded — an
            # identified user must never see unowned (pre-multi-user) rows.
    ...
    def get_messages(
        ...
        if include_compacted:
            ...
            with self._read_ctx() as conn:
                ...
                # A concurrent tail copied during compaction is left inactive
                # and non-compacted in the closed parent. It must not appear in
                # the display set, but a cursor minted against one of those
                # original rows still needs its logical key to resolve to the
                # fresh active clone. Read the wider row set only for that
                # identity mapping; visibility remains governed by
                # ``display_rows`` below.
            ...
            if before_id is not None:
                ...
                if superseded_key is not None and superseded_key in seen:
    ...
    def session_count(
        ...
        Pass ``include_hidden`` to mirror ``list_sessions_rich``'s
        ``include_hidden`` filter. Defaults to ``False`` (hidden sessions,
        e.g. Bot Mode, are excluded) so a caller pairing this with an
        ``include_hidden=False`` listing gets a ``total``/``has_more`` that
        matches the actual page; a caller that lists with
        ``include_hidden=True`` must pass the same flag here or the total
        will under-count.
        """
        where_clauses = []
        params = []
