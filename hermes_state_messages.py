...
class SessionMessagesMixin:
    ...
    def get_messages(
        ...
        after_id: Optional[int] = None, before_id: Optional[int] = None,
    ) -> List[Dict[str, Any]]:
        """Load messages in insertion order, with optional keyset paging.

        ``before_id`` returns the newest ``limit`` rows strictly older than the
        cursor, then restores chronological order. It is compatible with
        compacted display reads because the deduped display set is already
        materialized before the bound is applied. A cursor that names a row
        superseded by compaction is anchored to its surviving representative.
        """
        ...
        if include_compacted:
            display_rows = self._read_all(
                "SELECT * FROM messages WHERE session_id = ?" + active_clause + " ORDER BY id ASC", [session_id])
            rows = self._dedupe_display_generations(display_rows)
            if before_id is not None:
                # Reproduce the display dedupe key so a cursor from an archived
                # generation can follow its live clone.
                cursor_rows = display_rows if include_inactive else self._read_all(
                ...
                for row in cursor_rows:
                    ...
                    if row["role"] == "user":
                        ...
                        if handoff is not None and live_view is not None:
                            dedupe_content = self._encode_content(live_view.get("content"))
                    key = (row["role"], dedupe_content, row["timestamp"], row["tool_call_id"],
                ...
                for row in rows:
                    ...
                    if row["role"] == "user":
                        ...
                        if handoff is not None and live_view is not None:
                            dedupe_content = self._encode_content(live_view.get("content"))
                    selected_by_key[(row["role"], dedupe_content, row["timestamp"], row["tool_call_id"],
                                    row["tool_calls"], row["tool_name"])] = row
                anchor = before_id
                if before_id in key_by_id:
                    representative = selected_by_key.get(key_by_id[before_id])
                    if representative is not None:
                        anchor = representative["id"]
                rows = [row for row in rows if row["id"] < anchor]
                rows = rows[-limit:] if limit is not None and limit else ([] if limit == 0 else rows)
            else:
                if latest:
                    rows = rows[::-1]
                rows = rows[offset:]
            ...
            keyset_clause = " AND id > ?" if after_id is not None else (" AND id < ?" if before_id is not None else "")
            sql = (f"SELECT * FROM messages WHERE session_id = ?{active_clause}{keyset_clause} "
                   f"ORDER BY id {'DESC' if descending else 'ASC'}")
