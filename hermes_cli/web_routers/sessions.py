...
async def get_session_messages(
    ...
    """Page a session transcript, with an opaque cursor for older history."""
    if order not in (None, "oldest", "latest"):
        raise HTTPException(status_code=400, detail="order must be one of: oldest, latest")

    # Decode before opening or looking up a session. A malformed cursor must
    # not become a session-existence oracle.
    ...
    if before is not None:
        ...
        if offset:
            raise HTTPException(status_code=400, detail="before cannot be combined with offset")
    ...
    def _read(db):
        ...
        page_limit = 500 if default_page else min(limit, 500)
        if before_id is not None:
            latest_page = False
        overfetch = page_limit > 0 and (before_id is not None or latest_page)
        rows = db.get_messages(
            sid,
            limit=page_limit + 1 if overfetch else page_limit,
            offset=offset,
            latest=latest_page,
            include_compacted=include_compacted,
            before_id=before_id,
        )
        if overfetch and len(rows) > page_limit:
            return sid, page_limit, rows[1:], True
        older_exists = (not overfetch) and bool(rows) and offset > 0
        return sid, page_limit, rows, older_exists

    result = await asyncio.to_thread(_with_db, profile, _read, read_only=True)
    if result is None:
        raise HTTPException(status_code=404, detail=_NOT_FOUND)
    sid, page_limit, messages, older_exists = result
    projected_messages = _project_for_display(messages)
    oldest_id = projected_messages[0].get("id") if projected_messages else None
    ...
    return {
        ...
        "pagination": {
            "limit": page_limit,
            "offset": offset,
            "order": order or ("latest" if (limit is None or before_id is not None) else "oldest"),
