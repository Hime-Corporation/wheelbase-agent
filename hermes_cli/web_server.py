...
def mount_spa(application: FastAPI):
    ...
    # ``--skip-build``) may have no built web bundle. The asset mount below is
    # deliberately ``check_dir=False`` and the SPA checks the bundle on each
    # request, so a missing ``web_dist/assets`` directory cannot prevent the
    # child from starting or make routing fall back to the shared store.
