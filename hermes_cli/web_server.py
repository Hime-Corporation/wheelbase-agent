...
def mount_spa(application: FastAPI):
    ...
    #
    # Separately, a headless gateway (e.g. the per-profile child dashboard
    # spawned with ``--skip-build``) ships no built web bundle: ``web_dist``
    # may be absent OR present-but-incomplete (no ``assets/`` subdir). Either
    # way there is no usable SPA to serve, so run frontend-less — otherwise
    # the unconditional ``StaticFiles(directory=WEB_DIST / "assets")`` mount
    # below raises ``RuntimeError: Directory '.../web_dist/assets' does not
    # exist`` at startup and the child never becomes ready (issue: per-profile
    # routing silently fell back to the shared launch store).
    _headless = os.environ.get("HERMES_SERVE_HEADLESS") == "1"
    _no_dist = not WEB_DIST.exists() or not (WEB_DIST / "assets").is_dir()
    if _headless or _no_dist:
        _msg = (
