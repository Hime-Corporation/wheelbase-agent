...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    async def _handle_session_messages(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions/{session_id}/messages with backward cursor paging."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err

        # Decode before the session lookup so malformed cursors do not expose
        ...
        if raw_before is not None:
            try:
                ...
                return web.json_response(_openai_error(str(exc), code="invalid_cursor"), status=400)
        ...
        include_compacted = str(request.query.get("include_compacted", "")).strip().lower() in (
            "1", "true", "yes", "on")
        ...
        if offset < 0 or (requested_limit is not None and requested_limit < 0):
            return web.json_response(
                _openai_error("limit and offset must be non-negative integers", code="invalid_pagination"),
                status=400,
            )
        if before_id is not None and offset:
            return web.json_response(
                _openai_error("before cannot be combined with offset", code="invalid_pagination"),
                status=400,
            )
