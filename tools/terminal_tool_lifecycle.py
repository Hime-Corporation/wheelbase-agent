...
def _create_configured_env(
    ...
    return _create_environment(
        ...
        container_config=(
            _container_config_from_config(config, resolve_task_overrides(task_id))
    ...
    with _env_lock:
        ...
        for task_id in stale:
            ...
            if env is not None and getattr(env, "_always_on", False):
                # Daytona's always-on mode deliberately trades idle cost for
                # cross-turn warm state. Refresh its activity instead of
                # stopping the sandbox.
                _last_activity[task_id] = current_time
                continue
            envs_to_stop.append((task_id, _active_environments.pop(task_id, None)))
            _last_activity.pop(task_id, None)
        with _creation_locks_lock:
            for task_id, _env in envs_to_stop:
                _creation_locks.pop(task_id, None)
