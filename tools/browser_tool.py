...
def _get_cdp_override(task_id: str = "default") -> str:
    ...
    """
    raw = _get_cdp_override_raw(task_id)
    if not raw:
        return ""
...
def _navigation_session_key(task_id: str, url: str) -> str:
    ...
    if task_id is None:
        task_id = "default"
    if _get_cdp_override_raw(task_id):
        return task_id
    if _is_camofox_mode():
        return task_id
...
def _get_session_info(task_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if existing_session is not None:
        if (
            ...
            # wheelbase fork: a caller that needs the desktop CDP relay must
            # never be handed a detached session.
        ...
        with _cleanup_lock:
            replacement = _active_sessions.get(task_id)
        if replacement is not None and replacement is not existing_session:
            # wheelbase fork: the same relay guard applies to the replacement.
            if desktop_required and not replacement.get("cdp_url"):
                raise DesktopUnavailableError(
                    "active browser session is not attached to the desktop CDP relay"
                )
            return replacement
