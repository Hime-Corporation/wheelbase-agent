...
def _create_cloud_session_or_fallback(task_id: str, provider) -> Dict[str, Any]:
    ...
    try:
        ...
        if session_info.get("cdp_url"):
            ...
            resolved = _cdp._resolve_cdp_override(str(session_info["cdp_url"]))
            if not resolved:
                raise RuntimeError(
                    f"Cloud provider {type(provider).__name__} returned a cdp_url that failed discovery"
                )
...
def _create_session_for_key(task_id: str, force_local: bool) -> Dict[str, Any]:
    ...
    cdp_override = _cdp._get_cdp_override(task_id)
    if cdp_override and not force_local:
        try:
            return _create_cdp_session(task_id, cdp_override)
        except Exception:
            if _cdp._desktop_requires_cdp(task_id):
                raise
            _bt.logger.warning(
                "CDP override session for task %s failed; falling back to local Chromium",
...
def _get_session_info(task_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    desktop_required = _cdp._desktop_requires_cdp(task_id)
    if desktop_required and not _cdp._desktop_task_cdp_raw(task_id):
        raise _cdp.DesktopUnavailableError("desktop CDP relay was not advertised")
    ...
    if existing_session is not None:
        if desktop_required and not existing_session.get("cdp_url"):
            raise _cdp.DesktopUnavailableError(
    ...
    if desktop_required and force_local:
        raise _cdp.DesktopUnavailableError("desktop browser cannot use a local sidecar")
    try:
        session_info = _create_session_for_key(task_id, force_local)
    except Exception as exc:
        if desktop_required:
            raise _cdp.DesktopUnavailableError("desktop CDP session creation failed") from exc
        raise
...
def _run_browser_command(
    ...
    desktop_required = _cdp._desktop_requires_cdp(task_id or "default")
    if desktop_required:
        raw_cdp = _cdp._desktop_task_cdp_raw(task_id or "default")
        resolved_cdp = _cdp._resolve_cdp_override(raw_cdp) if raw_cdp else ""
        if not resolved_cdp:
            try:
                from wheelbase_sdk.runtime import desktop_unavailable_result

                return desktop_unavailable_result(
                    detail="desktop CDP relay discovery failed" if raw_cdp else "desktop CDP relay was not advertised"
                )
            except ImportError:
                return {"success": False, "code": "desktop_unavailable", "error": "desktop CDP relay unavailable"}
    ...
    if "browser_cmd" not in preflight:
        if desktop_required:
            try:
                ...
                return desktop_unavailable_result(detail=str(preflight.get("error") or "browser backend unavailable"))
            except ImportError:
                return {"success": False, "code": "desktop_unavailable", "error": str(preflight.get("error") or "browser backend unavailable")}
        return preflight
    ...
    try:
        ...
        if isinstance(e, _cdp.DesktopUnavailableError):
            try:
                from wheelbase_sdk.runtime import desktop_unavailable_result

                return desktop_unavailable_result(detail=e.detail)
            except ImportError:
                return {"success": False, "code": "desktop_unavailable", "error": e.detail or str(e)}
    ...
    if desktop_required and not result.get("success"):
        try:
            ...
            return desktop_unavailable_result(detail=str(result.get("error") or "desktop CDP command failed"))
        except ImportError:
            return {"success": False, "code": "desktop_unavailable", "error": str(result.get("error") or "desktop CDP command failed")}
