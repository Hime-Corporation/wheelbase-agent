...
def _container_config_from_config(
    ...
    *overrides* is the per-task/per-session override dict (see
    :func:`resolve_task_overrides`), e.g. registered by the cloud gateway for
    an authenticated user's stable sandbox. When present, its
    ``docker_volumes`` are APPENDED to the global volumes (so a per-user mount
    coexists with the operator's global mounts) and its ``docker_env`` values
    WIN over the global ``docker_env`` (so a per-user secret/setting can
    override a global default) — mirroring ``register_task_env_overrides``'s
    merge-not-replace contract.
    """
