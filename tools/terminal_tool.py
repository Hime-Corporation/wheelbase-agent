...
def register_task_env_overrides(task_id: str, overrides: Dict[str, Any]):
    ...
    """
    # Multiple runtime owners contribute disjoint pieces. Preserve the
    # existing registration when a later caller adds only cwd or relay data.
    _task_env_overrides[task_id] = {
        **_task_env_overrides.get(task_id, {}),
        **overrides,
    }
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    """
    if task_id:
        sandbox_key = _task_env_overrides.get(task_id, {}).get("sandbox_key")
        if sandbox_key:
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "daytona_always_on": _tenv_bool("TERMINAL_DAYTONA_ALWAYS_ON", "false"),
