    ...
    try:
        ...
        try:
            ...
            kernel_result = execute_in_remote_kernel(
                ...
                reset=bool(reset),
                idle_exit=int(_load_config().get("kernel_idle_timeout", 1800)),
            )
        except Exception:
            logger.warning(
    ...
    try:
        ...
        if not py3_ok:
            ...
            return _error_result(
                f"python3 did not actually run in the {env_type} terminal environment "
                f"(probe output: {detail}). Install a real Python 3 interpreter to use "
                "execute_code with remote backends."
            )
