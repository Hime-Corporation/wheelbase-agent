...
def _container_config_from_config(config: Dict[str, Any], overrides: Optional[Dict[str, Any]] = None) -> dict:
    """``container_config`` for :func:`_create_environment` (shared with the lazy ``ensure_task_env``)."""
    overrides = overrides or {}
    result = {k: config.get(k, d) for k, d in _CONTAINER_KEYS}
    result["docker_volumes"] = list(result.get("docker_volumes") or []) + list(overrides.get("docker_volumes") or [])
    result["docker_env"] = {**(result.get("docker_env") or {}), **(overrides.get("docker_env") or {})}
    if "daytona_always_on" in overrides:
        result["daytona_always_on"] = overrides["daytona_always_on"]
    return result
...
_SANDBOX_ROWS = {
    ...
    "daytona": (lambda: importlib.import_module("tools.environments.daytona").DaytonaEnvironment, True,
                lambda cc, kw: {"cpu": int(kw["cpu"]), "always_on": bool(cc.get("daytona_always_on", False))}),
