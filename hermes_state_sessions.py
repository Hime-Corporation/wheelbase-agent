...
def _session_filter_where(
    *, exclude_children: bool = False, source: str = None, sources: List[str] = None,
    ...
    if user_id:
        # An identified cloud connection must never see legacy unowned rows or
        # another user's sessions. The caller supplies the trusted identity.
...
""" + _UPSERT_KEEP_EXISTING_SQL,
    ...
    def list_sessions_rich(
        ...
        where_clauses, params = _session_filter_where(
            ...
            archived_only=archived_only, include_archived=include_archived, user_id=user_id,
        )
    ...
    def session_count(
        ...
        user_id: str = None, include_hidden: bool = False,
    ) -> int:
        """Count sessions with list_sessions_rich's filters so a paired "load more" total matches.

        ``user_id`` scopes identified cloud callers. ``include_hidden`` mirrors
        the list default, so totals do not include rows the picker cannot show.
        """
        where_clauses, params = _session_filter_where(
