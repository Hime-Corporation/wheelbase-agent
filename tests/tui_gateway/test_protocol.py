...
@pytest.fixture()
def capture(server):
    """Capture protocol frames for a minimal live synthetic session."""
    buf = io.StringIO()
    previous = server._sessions.pop("s1", None)
    # The Wheelbase emitter deliberately drops events for closed/nonexistent
    # runtime sessions. Capture-based protocol tests use ``s1`` as a live
    # synthetic session, so register its durable identity without starting an
    # agent or worker.
    server._sessions["s1"] = {"session_key": "s1", "transport": None}
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        server,
        "_emit_global_event",
        lambda ev, payload=None: emitted.append((ev, payload)),
    )
