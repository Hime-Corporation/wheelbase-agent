...
def test_session_resume_deferred_history_failure_can_retry(monkeypatch):
    ...
    try:
        ...
        # Upstream minted a fresh uuid4[:8] per resume, so a retry was
        # observable as a NEW sid. Wheelbase's runtime session id is the
        # durable session key, so the retry deliberately reuses it — that is
        # what lets wheelbase-app keep one id for a chat forever. The retry is
        # proven by the assertions around it instead: the failed attempt was
        # unregistered above, and this one hydrates and builds.
        assert second_sid == first_sid == "retry-session"
...
def test_ensure_session_db_row_persists_explicit_cwd(monkeypatch, tmp_path):
    ...
    class _FakeDB:
        def create_session(self, key, source=None, model=None, model_config=None, parent_session_id=None, cwd=None, user_id=None, profile_name=None):
            created.append(
...
def test_session_most_recent_returns_first_non_denied(monkeypatch):
    ...
    class _DB:
        def list_sessions_rich(self, *, source=None, limit=200, user_id=None, order_by_last_active=False, compact_rows=False):
            return [
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool_lifecycle": fake}):
        ...
        with (
            ...
            # Upstream's _failure_messages now builds the "no browser found" hint
            # from manual_chrome_debug_command (not get_chrome_debug_candidates);
            # force it empty so the no-supported-browser branch is exercised.
            patch(
...
def test_browser_manage_connect_preserves_cdp_capability_token(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}), \
            patch("tools.browser_tool_lifecycle.cleanup_all_browsers", lambda: None):
        urls = _stub_urlopen_capture(monkeypatch, ok=True)
        resp = server.handle_request(
            {
                "id": "1",
...
def test_workspace_move_refuses_running_session(monkeypatch, tmp_path):
    """A RUNNING session refuses the move — neither the row nor the runtime moves.

    Upstream dropped this refusal in #86626 so an explicit Move-to-project
    would re-anchor the row and the live session together. Wheelbase keeps it:
    a live turn's tools are already resolving paths against the current cwd,
    and swapping it mid-turn makes the running tool call write into a
    directory the model never saw. The refusal is the whole point — the caller
    retries once the turn ends, and both stores stay consistent because
    NEITHER was touched (the #86626 disagreement this guards against).
    """
    ...
    live = {"session_key": target, "running": True, "cwd": old_cwd}
    ...
    try:
        ...
        assert res["error"]["code"] == 4009, res
        # Fail closed on BOTH stores: a refusal that had already committed one
        # of them would recreate the exact UI-vs-db split #86626 described.
        assert "row_update" not in captured
        assert live["cwd"] == old_cwd
        assert live.get("explicit_cwd") is not True
    finally:
        server._sessions.pop("live-sid", None)


def test_workspace_move_rehomes_idle_session(monkeypatch, tmp_path):
    """With the turn finished, the move applies to the row and the runtime
    together — the #86626 invariant, on the sessions where it is safe."""
    target = "stored-idle-session"
    new_cwd = tmp_path / "dest-project"
    new_cwd.mkdir()
    captured = {}
    ...
    monkeypatch.setattr(
        server.git_probe, "common_repo_root", lambda cwd: str(new_cwd)
    )

    live = {"session_key": target, "running": False, "cwd": str(tmp_path / "old-project")}
    server._sessions["idle-sid"] = live
    monkeypatch.setattr(server, "_register_session_cwd", lambda _session: None)
