...
class TestMaybeAutoTitle:
    ...
    def test_instant_title_persists_a_duplicate_title_unchanged(self, tmp_path):
        """Duplicate titles are legal since schema v26: a colliding derived
        title is written as-is, not scanned into 'hi #2'.

        Common openers collide constantly. The real store has no unique-title
        index to collide against, so both sessions land on the exact same
        title and neither pays for a lineage scan on the turn's critical path.
        """
        db = SessionDB(tmp_path / "state.db")
        ...
        assert db.get_session_title("sess-1") == "hi"
        assert db.get_session_title("taken") == "hi"

    def test_llm_titles_can_duplicate_across_unrelated_sessions(self, tmp_path):
        """An LLM-upgraded title is exactly as legal to duplicate as a
        derived one — schema v26 has no unique-title index for either."""
        db = SessionDB(tmp_path / "state.db")
        db.create_session(session_id="sess-a", source="cli")
        db.create_session(session_id="sess-b", source="cli")
        with patch(
            "agent.title_generator.generate_title",
            return_value="Fix flaky auth test",
        ):
            auto_title_session(db, "sess-a", "fix the flaky auth test")
            auto_title_session(db, "sess-b", "fix the flaky auth test")
        assert db.get_session_title("sess-a") == "Fix flaky auth test"
        assert db.get_session_title("sess-b") == "Fix flaky auth test"
        assert db.get_session_title_source("sess-a") == "llm"
        assert db.get_session_title_source("sess-b") == "llm"
...
class TestAutoTitleDuplicateHandling:
    ...
    def test_background_stage_persists_a_duplicate_title_unchanged(
        self, tmp_path
    ):
        """The real store never needs the lineage scan: duplicates are legal.

        ``generate_title`` coming back empty falls back to the same derived
        title another session already carries. The real ``SessionDB`` has no
        unique-title index in schema v26, so it persists the duplicate as-is
        instead of raising ``ValueError`` and retrying with a ``#N`` suffix —
        that fallback only exists for a legacy/read-only store that still
        enforces uniqueness.
        """
        db = SessionDB(tmp_path / "state.db")
        db.create_session(session_id="taken", source="cli")
        ...
        db.create_session(session_id="sess-1", source="cli")
        with patch.object(
            db, "get_next_title_in_lineage", wraps=db.get_next_title_in_lineage
        ) as lineage_spy, patch(
            "agent.title_generator.generate_title", return_value=None
        ):
            auto_title_session(db, "sess-1", "hi")
        assert db.get_session_title("sess-1") == "hi"
        lineage_spy.assert_not_called()
