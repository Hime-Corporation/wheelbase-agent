...
def _cdp_http_reachable(parsed, timeout: float = 2.0) -> bool:
    ...
    for url in (f"{root}/json/version{suffix}", f"{root}/json{suffix}"):
...
def _normalize_cdp_url(parsed) -> str:
    """Collapse discovery-style CDP URLs while preserving capability queries."""
...
def _redact_browser_url(value: object) -> str:
    """Mask capability tokens before a browser-connect error reaches a client or log."""
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith("/devtools/browser/"):
            ...
            try:
                ...
                return _err(rid, 5031, f"could not reach browser CDP at {_redact_browser_url(url)}: "
