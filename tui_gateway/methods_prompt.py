...
def _(rid, params: dict) -> dict:
    ...
    if (limit_message := _ensure_active_session_slot(sid, session)) is not None:
        ...
        # Refuse before any session row, queue, or model state is mutated.
    ...
    # before anything mutates session state — client_surface, transport,
    # history, queue, or running-state — so a replay is a pure read.
    ...
    with session["history_lock"]:
        ...
        if (
            ...
            #
            # The agent's session handle owns this row on the cloud path; use
            # the process-wide handle only for local sessions without one. A
            # profile session (app-global remote mode) keeps its transcript in
            # its own profile state.db, so writing through the launch handle
            # would resurrect the undone turns on resume or copy them into a
            # foreign profile. Fail-closed only holds if the checked handle is
            # the one that owns the row.
            _agent_db = getattr(session.get("agent"), "_session_db", None)
            _edit_db_context = (
                contextlib.nullcontext(_agent_db)
                if _agent_db is not None
                else _session_db(session)
            )
            with _edit_db_context as db:
                if db is not None:
                    try:
    ...
    if turn_isolation:
        ...
        if not isolated_response.get("error"):
            ...
            # Remember the COMPLETE response (row-ID rebind included) so a
            # replay of this idempotency key returns the same payload a
    ...
    # is still spinning up must replay this ACK — including the row-ID rebind
    # payload — rather than queue a second turn.
...
def register(server) -> None:
    ...
    # sites) resolve the same free names after the split. server.py's run path
    # also calls _pending_reaction_notes directly (run_message enrichment,
    # beside the speech-interrupted note), and the idempotency helpers resolve
    # their free names through server's globals too — publish the TTL/cap
    # constants alongside (a rebind swaps __globals__ wholesale — nothing from
    # this module's namespace comes with them).
