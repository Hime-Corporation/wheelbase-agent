...
# ``prompt.submit`` is retried by clients when a WebSocket ACK is lost. The
# JSON-RPC id is only a correlation id, so remember accepted ACK payloads by
# the client's explicit idempotency key and replay them without starting a
# second turn.
...
def _prompt_idempotency_replay(session: dict, key: str) -> dict | None:
    ...
    with session["history_lock"]:
        ...
        if not isinstance(seen, dict):
            return None
        for stale in [
            candidate for candidate, entry in seen.items()
            if now - float((entry or {}).get("at") or 0) > _PROMPT_IDEMPOTENCY_TTL_SECS
        ]:
            seen.pop(stale, None)
        entry = seen.get(key)
        return dict(entry.get("result") or {}) if isinstance(entry, dict) else None
...
def _remember_prompt_idempotency(session: dict, key: str, response: dict | None) -> None:
    ...
    if not isinstance(result, dict):
        return
    with session["history_lock"]:
        seen = session.setdefault("_prompt_idempotency", {})
        if not isinstance(seen, dict):
            seen = {}
...
def _(rid, params: dict) -> dict:
    ...
    if idempotency_key:
        ...
        if replayed is not None:
            logger.info(
                "prompt.submit: duplicate idempotency key on session %s; replaying the original acknowledgement",
                sid,
            )
    ...
    turn_identity = _transport_identity()
    with session["history_lock"]:
        if t is not None:
            session["transport"] = t
    ...
    run_thread.start()
    response = _ok(rid, {"status": "streaming", **survivor_fields})
...
def _spawn_side_agent(
    ...
    def run():
        ...
        # Bug #50233: ephemeral agent threads don't inherit the session's HERMES_HOME
        # override (the ContextVar set on the session-create thread doesn't
        # propagate here). Re-bind it for the duration of the turn, then restore.
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                ...
                clear_ephemeral_task_state(task_id, hermes_home=profile_home)
