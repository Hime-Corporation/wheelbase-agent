...
def _enqueue_prompt(session: dict, text: Any, transport: Any, image_paths: list[str] | None = None) -> None:
    ...
    if (_identity := getattr(transport, "wheelbase_identity", None)) is not None:
        # Keep the identity that queued the turn with its transport. A later
        # reconnect must not run the drained prompt under another user.
