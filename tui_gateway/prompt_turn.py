...
def _prepare_turn_input(sid: str, session: dict, st: _TurnRun, text: Any, images: list[str]):
    ...
    # Cloud gateway: bind this turn's task_id to the authenticated session's
    # SDK credential, browser relay, and sandbox. Injection is deliberately
    # inside the turn scope and fails closed before the model is called.
    if (wb_ident := session.get("wheelbase_identity")) is not None:
        from tui_gateway.wheelbase_inject import apply_session_injection

        scopes.wheelbase_inject = apply_session_injection(
            session["session_key"],
            wb_ident,
            Path(get_hermes_home()),
            conversation_id=session["session_key"],
            explicit_cwd=_wheelbase_explicit_cwd(session),
            connection_id=_wheelbase_connection_id(session.get("transport")),
        )
...
def _finish_turn(sid: str, session: dict, st: _TurnRun) -> None:
    ...
    if scopes.wheelbase_inject is not None:
        with contextlib.suppress(Exception):
            scopes.wheelbase_inject()
