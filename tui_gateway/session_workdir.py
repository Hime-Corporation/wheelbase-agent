...
def _workdir_owner_db(session: dict, fail_log: str):
    ...
    agent_values = vars(agent) if hasattr(agent, "__dict__") else {}
    agent_db = agent_values.get("_session_db")
    if agent_db is not None:
        db = agent_db
    elif profile_home := session.get("profile_home"):
        try:
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    if session.get("wheelbase_identity") is not None:
        # Identified cloud sessions use the per-user sandbox workspace. The
        # client path may belong to another machine, so contain it before the
        # host-side isdir check below.
