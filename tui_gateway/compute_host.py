...
class ComputeHost:
    ...
    def _ensure_server_session(self, server: Any, frame: dict[str, Any]) -> dict:
        ...
        if session is not None:
            ...
            if isinstance(frame.get("wheelbase_identity"), dict):
                try:
                    from tui_gateway.wheelbase_identity import WheelbaseIdentity

                    identity = WheelbaseIdentity(**{
                        field: frame["wheelbase_identity"].get(field)
                        for field in WheelbaseIdentity.__dataclass_fields__
                        if field in frame["wheelbase_identity"]
                    })
                    self._transport.wheelbase_identity = identity
                    session["wheelbase_identity"] = identity
                except Exception:
                    pass
