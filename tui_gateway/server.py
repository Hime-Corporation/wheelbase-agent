...
def _identity_derived_profile() -> str | None:
    """``wb-<user_id>`` for the current connection's identity, else None.

    None covers both "no identity" (legacy desktop/TUI) and an identity
    with no ``user_id`` — both mean "no per-user profile to derive".
    """
    ...
    if ident is not None and ident.user_id:
        try:
            ...
            PROFILE_PREFIX = "wb-"
        return f"{PROFILE_PREFIX}{ident.user_id}"
    return None


def _request_profile(params: dict | None) -> str | None:
    """Resolve the per-profile name for a ``session.*`` RPC.

    The cloud gateway is a single machine dashboard serving every user via
    per-profile ``state.db`` files. An explicit ``profile`` param (app-global
    remote mode) always wins; otherwise derive ``wb-<user_id>`` from the
    CONNECTION IDENTITY — never trust an absent param to mean "shared launch
    store" once a connection is identified (mirrors ``_db_for_profile``).
...
def _db_for_profile(profile: str | None = None):
    ...
    Cloud gateway: with no explicit ``profile`` (every mobile client, and
    desktop in own-profile mode) derive ``wb-<user_id>`` from the CONNECTION
    IDENTITY (:func:`_identity_derived_profile`), mirroring session.create /
    session.resume — never trust an absent param to mean "shared launch
    store" once a connection is identified. FAIL CLOSED: when an identity is
    present and its derived profile home doesn't resolve (not yet
    provisioned, or any other lookup failure), raise instead of silently
    returning the shared store. Silently falling back here is precisely the
    cross-user session-bleed regression this fork exists to prevent
    (docs/upstream-merge-plan-2026-07-30.md §6.1) — it would land on every
    handler that resolves its db via this function or :func:`_profile_db`.

    Returns (db, owns_handle). ``db`` is None when unavailable (only possible
    on the non-identified / no-profile path).
    """
...
def _request_profile_db(params: dict):
    ...
    Alias of :func:`_profile_db` kept under the fork's historical name for
    handlers that look a session up by id (delete/most_recent/etc.) rather
    than holding a live session dict.
    """
...
def _ensure_session_db_row(session: dict) -> bool:
    ...
    * A terminal session (``hermes`` / ``hermes --tui`` / CLI) is started from a
    ...
    # Bot-Mode room plumbing sessions and canonical chats must follow the
    # member profile's current runtime configuration rather than a provider
    # pinned when their row was first written.
    ...
    # Cloud gateway: stamp the owning user on the row so list_sessions_rich /
    # session_count can scope per-user (see _db_for_profile's identity fallback).
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    if session.get("wheelbase_identity") is not None:
        ...
        # contain_workspace_path maps ANY client-supplied cwd into that sandbox
        # rather than resolving it as a host path. This must run BEFORE the
        # os.path.isdir()/ValueError check below: the raw client path (e.g. a
        # desktop-native path pasted from another OS) routinely doesn't exist
        # on THIS host, and upstream's newer isdir guard would reject it before
        # containment ever gets a chance to rewrite it.
...
def _tool_lifecycle_required_for_ui(name: str) -> bool:
    ...
    # ``todo`` fires unconditionally too, so clients always receive the latest
    # todo list regardless of whether tool_progress mode is enabled.
...
def _enqueue_prompt(
    ...
    if (
        ...
        prev = existing["text"]
        # Merging into the existing envelope deliberately keeps that envelope's
        # pinned transport/identity — the first submitter owns the drained turn.
...
def _drain_queued_prompt(rid, sid: str, session: dict) -> bool:
    ...
    with session["history_lock"]:
        ...
        # wheelbase fork: rebind the acting identity with the transport so the
        # drained turn runs as the user who queued it, not whoever last spoke.
        ...
        try:
            ...
            if _pending and status == "complete":
                ...
                try:
                    ...
                    # Non-retryable: the title/session key failed validation,
                    # or the target row lives in a legacy store that rejects
                    # this write. Duplicate titles are NOT an error here —
                    # Wheelbase titles are deliberately non-unique — so this
                    # branch is validation/legacy-store failures only.
                    # Fix for #19029.
                    session["pending_title"] = None
                    logger.info(
            ...
            # Auto-titling no longer runs here: it now fires once, at turn
            # start, from the shared prologue
            # (agent/turn_context.py:_maybe_title_session_at_turn_start),
            # wired to this session's live-rename hook via
            # `agent._on_session_title` above. A second call here would
            # launch a duplicate title attempt after every completed turn.
