...
def _transport_identity():
    """Return the identity attached to the currently bound transport, if any."""
    transport = current_transport()
    return getattr(transport, "wheelbase_identity", None) if transport is not None else None
...
def _wheelbase_connection_id(transport) -> str:
    ...
    if transport is None:
        return ""
    with _sessions_lock:
        connection_id = str(getattr(transport, "_wheelbase_connection_id", "") or "")
...
def _wheelbase_runtime_fingerprints(identity) -> dict[str, Any]:
    ...
    expected_tail = ("tenants", str(identity.tenant_id), "profiles", f"wb-{identity.user_id}")
    return {
...
def _identity_derived_profile() -> str | None:
    """Derive the Wheelbase profile from the authenticated connection identity."""
    ...
    try:
        ...
        PROFILE_PREFIX = "wb-"
    return f"{PROFILE_PREFIX}{identity.user_id}"


def _request_profile(params: dict | None) -> str | None:
    """Resolve and validate the profile selected by a JSON-RPC request.

    Identified cloud connections are already bound to one user profile and
    cannot override it through request parameters. Unidentified local callers
    retain explicit profile selection after name validation.
    """
...
def _profile_is_launch_home(profile: str | None) -> bool:
    """Distinguish the launch profile from a missing derived profile."""
    name = (profile or "").strip()
    if not name:
        return False
    try:
...
def _db_for_profile(profile: str | None = None):
    """Return ``(SessionDB, owns_handle)`` for a request's profile."""
    profile = (profile or "").strip() or None
    identified = False
    if profile is None:
        if (derived := _identity_derived_profile()) is not None:
            profile, identified = derived, True
...
def _request_profile_db(params: dict):
    """Yield the profile-scoped store selected by a session RPC."""
    db, owns = _db_for_profile(None if _identity_derived_profile() is not None else _request_profile(params))
    try:
        yield db
    finally:
        if owns and db is not None:
...
def _request_profile_home(params: dict) -> Path | None:
    """Resolve the selected request profile to its local home."""
    return _profile_home(_request_profile(params))
...
_GLOBAL_EVENT_TYPES = frozenset({
    "gateway.ready", "skin.changed", "pet.changed", "cron.changed",
    "sessions.changed", "platforms.changed", "pairing.changed", "session.reclaimed",
})


def _emit_session_event(event: str, runtime_sid: str, payload: dict | None = None):
    """Emit a session event with the durable ID while routing by the live runtime sid."""
    ...
    frame_sid = runtime_sid if getattr(transport, "_compute_host_transport", False) else durable_id
    frame = _event_frame(event, frame_sid, outward_payload)
    from tui_gateway.event_replay import _stamp_event
    _stamp_event(frame)
...
def _emit(event: str, sid: str, payload: dict | None = None):
    return _emit_session_event(event, sid, payload) if sid else _emit_global_event(event, payload)
