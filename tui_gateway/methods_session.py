...
def _(rid, params: dict) -> dict:
    ...
    # connection identity, mirroring session.list/session.resume. Without this,
    # a new chat persists its row to the machine-root state.db and builds its
    # agent against the machine-root HERMES_HOME, so it never appears in the
    # user's per-profile session.list and runs against the wrong profile's
    # skills/config — "new chat doesn't show up / wrong agent".
...
@method("session.list")
def _(rid, params: dict) -> dict:
    # _profile_db already derives wb-<user_id> from the connection identity
    # when no explicit ``profile`` param is given (see _db_for_profile).
    ...
    with _profile_db(params) as db:
        ...
        try:
            ...
            # ``title``: EXACT-title registry lookup, not a listing. Titles are
            # deliberately non-unique in Wheelbase, so SessionDB's deterministic
            # newest-row lookup supplies a stable answer for callers that treat
            # a title as an identity key (Bot Mode's canonical "Bot Chat" —
            # Profile → Named Session) without recreating the old unique-index
            # contract. Hidden rows resolve (canonical chats are born hidden);
            # archived rows and deny-listed sources do not; compression lineages
            # resolve to the live tip (``resolved_id``), mirroring
            # profiles.list's canonical_session resolver. Older clients never
            # send this param; newer clients falling back to older gateways just
            # get the normal windowed listing back (the param is ignored) and
            # scan it.
...
def _(rid, params: dict) -> dict:
    ...
    # mirroring session.list/session.create — the cloud gateway is a single
    # machine dashboard that serves all users via per-profile DB scoping.
    profile = _request_profile(params)
    profile_home = _profile_home(profile)
    ...
    try:
        ...
        # The identified connection's own transport identity — resolved BEFORE
        # any row lookup/fallback decision. It governs whether the legacy
        # title fallback below is reachable at all, and how the requested row
        # is ownership-gated once found.
        resume_ident = _transport_identity()

        def _resume_record_matches_identity(record: dict) -> bool:
            """Keep live-session fallbacks scoped to the authenticated user."""
            if resume_ident is None:
                return True
            observed = record.get("wheelbase_identity")
            if isinstance(observed, dict):
                observed_user_id = observed.get("user_id")
            else:
                observed_user_id = getattr(observed, "user_id", None)
            return bool(observed_user_id) and observed_user_id == resume_ident.user_id

        found = db.get_session(target)
        if not found:
            # Title lookup is a legacy convenience only. An identified
            # connection must resolve by durable id so duplicate titles cannot
            # select another user's session.
            found = db.get_session_by_title(target) if resume_ident is None else None
            if found:
                target = found["id"]
                ...
                if resume_ident is not None and not any(
                    isinstance(record, dict)
                    and record.get("session_key") == target
                    and _resume_record_matches_identity(record)
                ...
                def _find_live_unpersisted(needle: str, home) -> str:
                    ...
                    for live_sid, record in list(_sessions.items()):
                        ...
                        if (record.get("profile_home") or None) != want_home:
                            continue
                        if not _resume_record_matches_identity(record):
                            continue
                        if (
                ...
                if owns_db:
                    try:
                        ...
                        if donor_row and donor_row.get("archived"):
                            donor_row = None
                        if donor_row and resume_ident is not None and (
                            donor_row.get("user_id") or ""
                        ) != resume_ident.user_id:
                            donor_row = None
                        if donor_row:
        ...
        # A row adopted from another store is still untrusted until its owner
        # matches the authenticated transport identity. Foreign and legacy
        # NULL-owner rows deliberately return the same unavailable response.
        if resume_ident is not None and found and (
            found.get("user_id") or ""
        ) != resume_ident.user_id:
            return _err(rid, 4007, "session unavailable")
        ...
        # continuation child; without this, resuming the original id reloads the
        # parent transcript and the response generated after compression is
        # missing. Resolving here re-anchors the fast path below so a still-live
        # rotated session is reused instead of rebuilding a duplicate agent on
        # the stale parent. Skipped for lazy watch windows, which intentionally
        # attach to the exact child branch they were opened on. Bot Chat is a
        # named registry row, so it stays on a proven compression edge; other
        # sessions keep the legacy unmarked-child walker.
        #
        # The requested row is ownership-gated above, and the resolved tip must
        # carry the same owner. A foreign or legacy NULL-owner tip is ignored so
        # resume never crosses a Wheelbase identity or tenant boundary.
        ...
        # actually load before any reopen/history read. Only the non-deferred,
        # non-omitted resume reads the whole compression lineage into memory; the
        # deferred Desktop, omit_messages, and lazy watch paths load the tip
        # segment only. The metadata fallback keeps lightweight test/adaptor DBs
        # compatible. The limit resolves from config (0 disables) and runs after
        # ownership/tip resolution, so it bounds size but never decides access.
        ...
        if is_truthy_value(params.get("lazy", False)):
            # Durable runtime id: the public/runtime session id IS the Hermes
            # session key on the Wheelbase surface — never a fresh short-lived
            # UUID — so REST/desktop callers that cached the durable id keep
            # working against this live registration.
            sid = target
            source = _resolve_session_source(str(params.get("source") or "").strip() or None)
            lease = None  # claimed lazily on the first turn (_ensure_active_session_slot)
            try:
            ...
            # Stamp the resuming connection's identity on the record BEFORE it
            # can be claimed live. Every deferred/lazy/eager branch does this so
            # a later identity check on this session key (session.workspace.move,
            # a future turn) sees it from the moment the record exists rather
            # than only once an agent has been built.
            record["wheelbase_identity"] = resume_ident
        ...
        if defer_history and not is_truthy_value(params.get("eager_build", False)):
            # Durable runtime id, same as every other branch in this handler.
            sid = target
            source = _resolve_session_source(str(params.get("source") or "").strip() or None)
            lease = None  # claimed lazily on the first turn (_ensure_active_session_slot)
            _enable_gateway_prompts()
            ...
            # The hydration worker now owns a profile-scoped dedicated handle and
            # closes it after the transcript read; the shared launch DB is passed
            # with close_db=False (owns_db is already False for it) and remains
            # process-owned. Clearing owns_db here means the outer finally below
            # never races the worker for the same handle.
            if owns_db:
                owns_db = False
        ...
        # Durable runtime id: the eager path keeps sid == target too, same as
        # every deferred branch above.
        sid = target
        source = _resolve_session_source(str(params.get("source") or "").strip() or None)
        lease = None  # claimed lazily on the first turn (_ensure_active_session_slot)
        _enable_gateway_prompts()
        ...
        with _session_resume_lock:
            ...
            try:
                ...
                try:
                    ...
                    #
                    # Gated on owns_db: only a DEDICATED profile handle may be
                    # transferred. The shared launch db (owns_db False the
                    # whole way through) is process-owned and outlives every
                    # agent — marking it agent-owned here would make some
                    # later session.close() on ANY session sharing it call
                    # agent.close() -> close the launch db out from under every
                    # other live session.
                    #
                    # Past this line (when the transfer runs) the session is
        ...
        # deferred lazy/defer-history/cold-default returns, and the
        # double-checked-locking discard. Dropping it merely relied on
        # refcounting to release the sqlite fds; that stops being true the moment
        # anything pins the instance — SessionDB pins ITSELF once its background
        # token writer starts, via atexit.register(_drain_token_queue_at_exit)
        # (hermes_state.py), which only close() unregisters. A pinned handle
        # keeps its db/-wal/-shm fds and its writer thread for the life of the
        # process. owns_db is False here for: the shared launch db (never True),
        # a dedicated handle transferred to the hydration worker (defer_history),
        # and a dedicated handle transferred to the agent (eager success) — in
        # every one of those cases something else now owns the close.
...
def _(rid, params: dict) -> dict:
    ...
    move_ident = _transport_identity()
    if move_ident is not None:
        # Cloud gateway: an identified session is sandboxed under /workspace,
        # so contain the client path instead of resolving it as a host path —
        # the same branch _set_session_cwd takes. This must run INSTEAD OF the
        # os.path.isdir() probe below, not before it: a sandbox path (or a
        # desktop-native path pasted from another OS) routinely does not exist
        # on THIS host, so the host probe would reject every legitimate tenant
        # move. Git metadata is left empty rather than probed, because probing
        # resolves against the GATEWAY's filesystem — it would both mis-group
        # the session under a host repo and hand the tenant back host repo
        # layout it has no business seeing.
        from tui_gateway.wheelbase_inject import contain_workspace_path

        try:
            resolved = contain_workspace_path(raw)
        except ValueError as e:
            return _err(rid, 4017, str(e))
        branch, root = "", ""
    else:
        ...
        resolved = os.path.abspath(
            os.path.expanduser(translate_cwd_for_wsl_backend(raw))
        )
    ...
    with _sessions_lock:
        for sid, sess in list(_sessions.items()):
            if sess.get("session_key") != target:
                continue
            # Never re-home (or emit session.info about) another identity's
            # live session. Session keys are timestamp + 24 bits of hex, so an
            # unfiltered match is brute-forceable; this is the same ownership
            # gate session.resume applies before it resolves any lineage.
            if move_ident is not None:
                sess_ident = sess.get("wheelbase_identity")
                if (
                    sess_ident is None
                    or getattr(sess_ident, "user_id", "") != move_ident.user_id
                ):
                    continue
    ...
    with _profile_db(params) as db:
        ...
        # Ownership gate on the PERSISTED row — same shape as session.resume's:
        # an identified connection may only move a row it owns, and a foreign
        # or legacy NULL-owner row must fail exactly like a missing one so a
        # guessed session key can't be probed for existence. This runs BEFORE
        # either the live re-home or the db write below, not after.
        if row is not None and move_ident is not None and (
            row.get("user_id") or ""
        ) != move_ident.user_id:
            return _err(rid, 4007, "session not found")

        # Re-home the live runtime BEFORE committing the row. update_session_cwd
        # commits, so doing it first means a refusal here leaves the stored
        # session pointing at a workspace the runtime never accepted.
        if live is not None:
            try:
...
def _(rid, params: dict) -> dict:
    ...
    # / _profile_db's identity fallback) — else deletes 404 on the dashboard.
