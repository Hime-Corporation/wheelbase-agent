...
def _session_row_summary(row: dict, *, tip_row: dict | None = None, resolved_id=None) -> dict:
    ...
    return {"id": row["id"], **({} if resolved_id is None else {"resolved_id": resolved_id}),
            ...
            "lineage_root_id": row.get("_lineage_root_id") or None}
...
def _(rid, params: dict) -> dict:
    ...
    profile_home = _profile_home(profile := _request_profile(params))
...
def _(rid, params: dict, db) -> dict:
    try:
        ...
        rows = _listing_rows(
            db, max(limit * 2, 200), offset=offset, user_id=user_id,
            include_hidden=include_hidden,
        )[:limit]
        total = db.session_count(
            exclude_children=True,
            exclude_sources=list(_LISTING_DENY_SOURCES),
            user_id=user_id,
            include_hidden=include_hidden,
        )
        return _ok(rid, {
            "sessions": [_session_row_summary(s) for s in rows],
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if "cdp" in lost_surfaces:
            ...
            parsed = browser_tool_session._run_browser_command(task_id, "snapshot", [])
...
class _Resume:
    ...
    def __init__(self, rid, params: dict, target: str) -> None:
        ...
        self.profile = _request_profile(params)
    ...
    def mint(self, prompts: bool = True) -> tuple:
        """``(durable sid, source, cwd)`` for the live record this resume registers."""
        source = _resolve_session_source(_str_param(self.params, "source") or None)
        if prompts:
            _enable_gateway_prompts()
        return self.target, source, self.profile_resume_cwd or _default_session_cwd()
    ...
    def record(self, source: str, cwd: str, history: list, overrides: dict | None = None, **extra) -> dict:
        ...
        if (ident := _transport_identity()) is not None:
            record["wheelbase_identity"] = ident
        return record
...
def _find_live_unpersisted(needle: str, home) -> str:
    ...
    return next((
        ...
        and (ident is None or getattr(record.get("wheelbase_identity"), "user_id", "") == ident.user_id)
...
def _resume_adopt_stranded(ctx: _Resume) -> None:
    ...
    try:
        ...
        if not donor_row or donor_row.get("archived"):
            return
        ident = _transport_identity()
        if ident is not None and (donor_row.get("user_id") or "") != ident.user_id:
            return
...
def _resume_locate(ctx: _Resume) -> dict | None:
    ...
    if ctx.found:
        ident = _transport_identity()
        if ident is not None and (ctx.found.get("user_id") or "") != ident.user_id:
            ctx.found = None
            return _err(ctx.rid, 4007, "session unavailable")
        return None
    ident = _transport_identity()
    if ctx.lazy and _child_run_active(ctx.target):
        ...
        if ident is not None and not any(
            ...
            and (record.get("profile_home") or None) == (str(ctx.profile_home) if ctx.profile_home else None)
            and str(record.get("session_key") or "") == ctx.target
            and getattr(record.get("wheelbase_identity"), "user_id", "") == ident.user_id
    ...
    return None if ctx.found else _err(ctx.rid, 4007, "session unavailable")
...
def _resume_follow_tip(ctx: _Resume) -> None:
    ...
    if tip and tip != ctx.target:
        tip_row = ctx.db.get_session(tip)
        ident = _transport_identity()
        if tip_row is not None and (ident is None or (tip_row.get("user_id") or "") == ident.user_id):
            ctx.target = tip
            ctx.found = tip_row
...
def _(rid, params: dict) -> dict:
    """Re-home a stored session, preserving identity and mid-turn safety."""
    target = _str_param(params, "session_key")
    if not target:
        return _err(rid, 4007, "session_key required")
    raw = _str_param(params, "cwd")
    if not raw:
        return _err(rid, 4016, "cwd required")
    ...
    with _sessions_lock:
        ...
        for sid, sess in list(_sessions.items()):
            ...
            if move_ident is not None:
                ...
                if sess_ident is None or getattr(sess_ident, "user_id", "") != move_ident.user_id:
                    continue
    ...
    with _profile_db(params) as db:
        ...
        if row is not None and move_ident is not None and (row.get("user_id") or "") != move_ident.user_id:
            return _err(rid, 4007, "session not found")
        if live is not None:
            try:
        ...
        if row is not None:
            try:
                ...
                return _err(rid, 5007, f"move failed: {exc}")
    if live is not None:
