...
def _(rid, params: dict) -> dict:
    ...
    # commits, so doing it first means a refusal here leaves the stored session
    # pointing at a workspace the runtime never accepted.
    if live is not None:
        try:
    ...
    with _profile_db(params) as db:
        ...
        # A brand-new draft has no persisted row yet; the live re-home above
