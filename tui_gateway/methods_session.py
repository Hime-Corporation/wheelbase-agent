...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if not found:
            # Title fallback is a LEGACY convenience for local/unidentified
            # connections only — session titles are deliberately non-unique, so
            # resolving one is inherently ambiguous. An identified connection
            # must resolve strictly by durable id; falling back to title there
            # would let one user's guess land on another user's session merely
            # because they happened to pick the same title.
        ...
        # always gated directly, before any lineage/tip resolution — a caller
        # can never reach a row it does not own by handing us an id whose
